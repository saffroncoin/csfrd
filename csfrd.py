...
def format_bet (bet):
    ...
    return [util.BET_TYPE_NAME[bet['bet_type']], bet['feed_address'], util.isodt(bet['deadline']), target_value, leverage, str(bet['wager_remaining'] / config.UNIT) + ' cSFR', util.devise(db, odds, 'odds', 'output'), bet['expire_index'] - util.last_block(db)['block_index'], bet['tx_hash']]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backend-rpc-connect', help='the hostname or IP of the backend saffroncoind JSON-RPC server')
    ...
    parser_issuance.add_argument('--call-price', help='the price, in cSFR per whole unit, at which a callable asset may be called back (must agree with previous issuances)')
    ...
    parser_bet.add_argument('--wager', required=True, help='the quantity of cSFR to wager')
    parser_bet.add_argument('--counterwager', required=True, help='the minimum quantity of cSFR to be wagered by the user to bet against you, if he were to accept the whole thing')
    ...
    parser_dividend.add_argument('--quantity-per-unit', required=True, help='the quantity of cSFR to be paid per whole unit held of ASSET')
    ...
    parser_burn = subparsers.add_parser('burn', help='destroy {} tm earn cSFR, during an initial period of time')
    ...
    parser_rps.add_argument('--wager', required=True, help='the quantity of cSFR to wager')
    ...
    parser_rps.add_argument('--fee', help='the exact SFR fee to be paid to miners')
    ...
    parser_rpsresolve.add_argument('--fee', help='the exact SFR fee to be paid to miners')
    ...
    # Database
    logging.info('Status: Running v{} of csfrd.'.format(config.VERSION_STRING, config.XCP_CLIENT))
    ...
    if args.action == 'send':
        ...
        if args.fee: args.fee = util.devise(db, args.fee, 'SFR', 'input')
        wager = util.devise(db, args.wager, 'cSFR', 'input')
    ...
    # VIEWING (temporary)
        ...
        call_price = str(results['call_price']) + ' cSFR' if results['call_price'] else results['call_price']
