...
def match (db, tx, block_index=None):
    ...
    for tx0 in order_matches:
        ...
        if tx0_price > tx1_inverse_price:
            ...
            if block_index >= 210900 or config.TESTNET: # Protocol change.
                min_btc_quantity = 0.001 * config.UNIT  # 0.001 SFR
            ...
            # Debit the order, even if it involves giving saffroncoins, and so one
