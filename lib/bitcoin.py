...
def bitcoind_check (db):
    ...
    if time_behind > 60 * 60 * 2:   # Two hours.
        raise exceptions.BitcoindError('Saffroncoind is running about {} seconds behind.'.format(round(time_behind)))
...
def connect (host, payload, headers):
    ...
    for i in range(TRIES):
        try:
            ...
            logging.debug('Could not connect to Saffroncoind. (Try {}/{})'.format(i+1, TRIES))
            time.sleep(5)
    return None
...
def wallet_unlock ():
    ...
    if 'unlocked_until' in getinfo:
        if getinfo['unlocked_until'] >= 60:
            ...
            passphrase = getpass.getpass('Enter your Saffroncoind[‐Qt] wallet passhrase: ')
...
def transaction (tx_info, encoding='auto', fee_per_kb=config.DEFAULT_FEE_PER_KB,
    ...
    for address in destinations + [source]:
        if address:
            try:
                ...
                raise exceptions.AddressError('Invalid Saffroncoin address:', address)

    # Check that the source is in wallet.
    if not config.UNITTEST and encoding in ('multisig') and not public_key:
        if not rpc('validateaddress', [source])['ismine']:
            raise exceptions.AddressError('Not one of your Saffroncoin addresses:', source)
    ...
    for coin in unspent:
        ...
        logging.debug('Change quantity: {} cSFR'.format(change_quantity / config.UNIT))
    ...
    if not sufficient_funds:
        ...
        raise exceptions.BalanceError('Insufficient saffroncoins at address {}. (Need approximately {} {}.) To spend unconfirmed coins, use the flag `--unconfirmed`.'.format(source, total_btc_out / config.UNIT, config.BTC))
...
def sign_tx (unsigned_tx_hex, private_key_wif=None):
    ...
    if private_key_wif:
        ...
        if result['complete']:
            ...
            raise exceptions.TransactionError('Could not sign transaction with Saffroncoin Core.')
