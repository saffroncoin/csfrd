        ...
        if category == 'credits':
            ...
            if bindings['transfer']:
                ...
                if bindings['callable'] and (bindings['block_index'] > 283271 or config.TESTNET):   # Protocol change.
                    callability = 'callable from {} for {} cSFR/{}'.format(isodt(bindings['call_date']), bindings['call_price'], bindings['asset'])
                else:
            ...
            log_message = 'RPS: {} opens game with {} possible moves and a wager of {}'.format(bindings['source'], bindings['possible_moves'], output(bindings['wager'], 'cSFR'))
            logging.info(log_message)

        elif category == 'rps_matches':
            log_message = 'RPS Match: {} is playing a {}-moves game with {} with a wager of {} ({}) [{}]'.format(bindings['tx0_address'], bindings['possible_moves'], bindings['tx1_address'], output(bindings['wager'], 'cSFR'), bindings['id'], bindings['status'])
            logging.info(log_message)
            ...
            if bindings['status'] == 'valid':
                ...
                log_message = 'RPS Resolved: {} is playing {} on a {}-moves game with {} with a wager of {} ({}) [{}]'.format(rps_match['tx0_address'], bindings['move'], rps_match['possible_moves'], rps_match['tx1_address'], output(rps_match['wager'], 'cSFR'), rps_match['id'], rps_match['status'])
            else:
...
def version_check (db):
    try:
        host = 'https://raw2.github.com/saffroncoin/csfrd/master/version.json'
    ...
    if not passed:
        explanation = 'Your version of csfrd is v{}, but, as of block {}, the minimum version is v{}.{}.{}. Reason: ‘{}’. Please upgrade to the latest version and restart the server.'.format(config.VERSION_STRING, versions['block_index'], versions['minimum_version_major'], versions['minimum_version_minor'], versions['minimum_version_revision'], versions['reason'])
...
def database_check (db, blockcount):
    """Checks {} database to see if the {} server has caught up with Saffroncoind.""".format(config.XCP_NAME, config.XCP_CLIENT)
    if last_block(db)['block_index'] + 1 < blockcount:
        raise exceptions.DatabaseError('{} database is behind Saffroncoind. Is the {} server running?'.format(config.XCP_NAME, config.XCP_CLIENT))
    return
...
def debit (db, block_index, address, asset, quantity, action=None, event=None):
    ...
    if asset == config.BTC:
        raise exceptions.BalanceError('Cannot debit saffroncoins from a {} address!'.format(config.XCP_NAME))
